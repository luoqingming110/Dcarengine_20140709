...
namespace Dcarengine.Function_Class
{
    class Tp_KeyMethodFuntion
    {
        ...
        /// <returns></returns>
        private   static UInt32 KEYMethod(UInt32 num)
        {                    
            UInt32 key;
            UInt32 a = num;
            if (CommonConstant.mode != null || !"".Equals(CommonConstant.mode))
            {
                Hashtable keyTable = ModeKeyMap.GetHashtable(EcuVersionF.EcuVsion);
                String keyValue =Convert.ToString(keyTable[CommonConstant.mode]);
        ...
        /// con
        /// </summary>
        public    void  ConThread()
        {
            try
            {
                while (threadFlag == 0) { 

                   new Thread(confuntion).Start();               
                }
